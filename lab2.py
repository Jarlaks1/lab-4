...
def MixColumnsStep(line, x1, x2, x3, x4):
    return s_mul(line[0], x1) ^ s_mul(line[1], x2) ^ s_mul(line[2], x3) ^ s_mul(line[3], x4)
...
def Cipher(in_, w):
    ...
    for round in range(1, 10):
        ...
        state = AddRoundKey(state, w[round*4:4 * (round + 1)])

    state = SubBytes(state)
    state = ShiftRows(state)
    state = AddRoundKey(state, w[10*4:(10 + 1) * 4])
...
def InvCipher(in_, w):
    ...
    for round in range(10 - 1, 0, -1):
        ...
        state = AddRoundKey(state, w[4 * round:4 * (round + 1)])
...
def AES_main(data, key, mode):
    if len(key) != 16:
        raise Exception("Incorrect key size error!")
    ...
    result = process_(data, key, encryption)
...
def process_(data, key, encryption):
...
if __name__ == "__main__":
    ...
    res_ = AES_main(args.in_.read(), args.key.read(), args.mode)
